  ...
  return (
    <div className="stat-card relative overflow-hidden group">
      <div
        ...
        style={{ background: accent || '#ff6b00' }}
      />
          ...
          {[['gap', 'Gap Score'], ['searches', 'Searches'], ['listings', 'Listings']].map(([key, label]) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                sort === key ? 'bg-brand text-white' : 'text-muted hover:text-cream'
              }`}
            >
              {label}
            </button>
              ...
              <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    background: u.gap_score >= 70 ? '#EF4444' : u.gap_score >= 40 ? '#F59E0B' : '#ff6b00',
                  }}
                />
              </div>
            ...
            <div key={s.label}>
              ...
              <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${pct}%`,
                    background: i === 0 ? '#ff6b00' : i === 1 ? '#F4A944' : i === 2 ? '#3B82F6' : '#EC4899',
                  }}
                />
              </div>
            </div>
    ...
    <div className="card p-5">
      ...
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={monthly} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
          ...
          />
          <Area type="monotone" dataKey="revenue" stroke="#ff6b00" strokeWidth={2} fill="url(#revGrad2)" dot={false} />
...
const MOCK_CLUSTERS = [
  ...
  { stage: 'Locked In',    count: 6,  color: '#ff6b00' },
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard label="Total Revenue"      value={isLoading ? '—' : fmt(finance?.summary?.total_gross)}           sub="All time"             trend={12}  icon="💰" accent="#ff6b00" />
        <StatCard label="Unilo Commission"   value={isLoading ? '—' : fmt(finance?.summary?.total_commission)}      sub="5% of gross"          trend={8}   icon="📊" accent="#F4A944" />
        <StatCard label="Active Listings"    value={isLoading ? '—' : (finance?.summary?.active_listings ?? '—')}   sub="Approved & visible"   trend={-3}  icon="🏠" accent="#3B82F6" />
        <StatCard label="Cluster Pipeline"   value={isLoading ? '—' : (intelligence?.clusters?.reduce((a, c) => a + c.count, 0) ?? 33)} sub="Across all stages" trend={24} icon="🔗" accent="#8B5CF6" />
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {(behaviour?.top_searches?.slice(0, 20) || MOCK_SEARCHES).map((s, i) => (
            <span
              ...
              style={{
                background: `rgba(255, 107, 0, ${0.05 + (1 - i / 20) * 0.1})`,
                borderColor: `rgba(255, 107, 0, ${0.1 + (1 - i / 20) * 0.2})`,
                color: i < 5 ? '#ff6b00' : '#8A9BB0',
